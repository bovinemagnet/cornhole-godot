...
func test_query_returns_props_in_radius() -> String:
	...
	var result: PackedInt32Array = grid.query_radius_candidates(Vector2(0.0, 0.0), 4.0)
...
func test_query_excludes_distant_props() -> String:
	...
	var result: PackedInt32Array = grid.query_radius_candidates(Vector2(0.0, 0.0), 3.0)
	if result.size() != 0:
...
func test_query_at_corner_returns_local_props() -> String:
	...
	var result: PackedInt32Array = grid.query_radius_candidates(Vector2(-49.0, -49.0), 4.0)
	var ids := _sorted(result)
...
func test_query_zero_radius_returns_same_cell_indices() -> String:
	...
	var result: PackedInt32Array = grid.query_radius_candidates(Vector2(7.5, 7.5), 0.0)
	var ids := _sorted(result)
...
func test_query_returns_empty_for_empty_grid() -> String:
	...
	var result: PackedInt32Array = grid.query_radius_candidates(Vector2.ZERO, 100.0)
	if result.size() != 0:
...
func test_query_large_radius_returns_all_props() -> String:
	...
	var result: PackedInt32Array = grid.query_radius_candidates(Vector2.ZERO, 200.0)
