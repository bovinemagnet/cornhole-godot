...
## Simple Godot 4 helper for loading the Candy Land map JSON files.
## Usage:
##   var catalog := CandyMapLoader.load_catalog()
##   var data := CandyMapLoader.load_map("res://resources/maps/candy_land/maps/json/map_01_lollipop_lawn.json")
...
static func load_catalog(path: String = "res://resources/maps/candy_land/candy_land_catalog.json") -> Dictionary:
    var data: Variant = load_json(path)
    if typeof(data) != TYPE_DICTIONARY:
        return {}
    return data

static func load_manifest(path: String = "res://resources/maps/candy_land/maps/manifest.json") -> Dictionary:
    var data: Variant = load_json(path)
    if typeof(data) != TYPE_DICTIONARY:
        return {}
    return data
