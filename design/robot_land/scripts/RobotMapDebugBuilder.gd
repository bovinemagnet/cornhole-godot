...
extends Node2D
class_name RobotMapDebugBuilder
...
func _ready() -> void:
    if map_json_path != "":
        map_data = RobotMapLoader.load_map(map_json_path)
        queue_redraw()
...
func _draw() -> void:
    ...
    var color_by_type = {
        ...
        "maze_lane": Color(0.4, 1.0, 1.0, 0.25),
        "landing_pad": Color(1.0, 0.5, 0.2, 0.25),
        "road_loop": Color(0.7, 0.9, 0.7, 0.25),
        "residential": Color(0.8, 0.8, 1.0, 0.25)
    }
